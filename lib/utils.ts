...
/** CTC is stored as a bare number string ("2", "2.3") — some candidates type
 *  the full annual figure in rupees (e.g. "500000") instead of LPA ("5")
 *  since the field is free text — no real LPA is ever >= 1000, so normalize
 *  anything that large down to LPA. Returns null if nothing numeric is found. */
export const parseCtcLpa = (value: string | undefined): number | null => {
  const v = (value ?? '').trim();
  if (!v) return null;
...
/** Formats a stored CTC value as "X LPA", normalizing scale via `parseCtcLpa`
 *  (see there for why). Falls back to the raw string if nothing numeric is
 *  found — some older/manual entries already say "12 LPA" as free text. */
export const formatCtc = (value: string | undefined): string => {
  const v = (value ?? '').trim();
  if (!v) return '—';
  const num = parseCtcLpa(value);
  if (num === null) return /lpa/i.test(v) ? v : `${v} LPA`;
