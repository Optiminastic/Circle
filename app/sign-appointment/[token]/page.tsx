...
export default function SignAppointmentPage() {
  ...
  const upload = async () => {
    ...
    setError('');
    try {
      await uploadRequestDocument({ token, docType: SIGNED_APPOINTMENT_DOC, file });
      setPhase('done');
    } catch (e) {
        ...
        <div className="rounded-2xl border border-[#E4E6EA] bg-white p-6 shadow-sm">
          ...
          {phase === 'done' && (
            <div className="mt-6 flex flex-col items-center gap-2 py-6 text-center text-gray-600">
              ...
              <p className="text-sm">Your signed appointment letter has been received. You can close this page.</p>
            </div>
          )}
